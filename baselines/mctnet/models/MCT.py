...
class MCT(nn.Module):
    def __init__(self,
        ...
        # conv3 sees cat(f2, x_hr, d) => 2*n_bands + n_select_bands (MSI channels).
        mct_in_channels = int(2 * int(n_bands) + int(n_select_bands))
        self.conv3_in_channels = mct_in_channels
        self.conv3 = nn.Sequential(
            nn.Conv2d(mct_in_channels, n_bands, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv2d(n_bands, n_bands, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
        )
