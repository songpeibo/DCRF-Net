# CNMF_py3.py — Python 3 port for paper2 baselines (source: CNMF.py; CNMF.py unchanged).
# =============================================================================
...
def _cnmf_py3_fusion_validate(HSI, MSI, verbose):
    """Validate HWC inputs; return integer spatial scale w = HR / LR (strict exact multiple)."""
    if HSI.ndim != 3:
        raise ValueError(f"HSI must be ndarray [h_lr, w_lr, C], got shape {tuple(HSI.shape)}")
    if MSI.ndim != 3:
        raise ValueError(f"MSI must be ndarray [h_hr, w_hr, c], got shape {tuple(MSI.shape)}")
    rows1, cols1 = int(MSI.shape[0]), int(MSI.shape[1])
    rows2, cols2 = int(HSI.shape[0]), int(HSI.shape[1])
    if rows2 < 1 or cols2 < 1:
        raise ValueError(f"Invalid LR shape {rows2}x{cols2}")
    if rows1 % rows2 != 0 or cols1 % cols2 != 0:
        raise ValueError(
            f"CNMF requires HR spatial sizes to be exact integer multiples of LR: "
            f"HR {rows1}x{cols1}, LR {rows2}x{cols2}. "
            f"Crop HR (and matching GT) to H=h_lr*w, W=w_lr*w in the adapter before calling CNMF_fusion."
        )
    w_r = rows1 // rows2
    w_c = cols1 // cols2
    if w_r != w_c:
        raise ValueError(
            f"Inconsistent HR/LR integer scale: rows imply w={w_r}, cols imply w={w_c} "
            f"(HR {rows1}x{cols1}, LR {rows2}x{cols2})"
        )
    w_int = int(w_r)
    if w_int < 1:
        raise ValueError(f"Invalid scale w={w_int} from HR/LR sizes")
    return w_int

def CNMF_fusion(HSI,MSI,mask=0,verbose='off',MEMs=0,srf_prior=None):
    '''
    ...
    INPUT
        ...
        srf_prior : (optional) Nominal relative spectral response matrix ``(bands1, bands2)``
                    (MSI bands × HSI bands). When set, used instead of ``estR`` so fusion does
                    not implicitly fit the SRF from mismatched MSI/HSI observations.
    ...
    w = _cnmf_py3_fusion_validate(HSI, MSI, verbose)

    # Estimation of R (relative spectral responses) or use a fixed nominal prior matrix.
    if srf_prior is not None:
        R_fix = np.asarray(srf_prior, dtype=np.float64)
        if R_fix.ndim != 2 or R_fix.shape[0] != bands1 or R_fix.shape[1] != bands2:
            raise ValueError(
                f"srf_prior must have shape ({bands1}, {bands2}) (MSI×HSI bands), got {tuple(R_fix.shape)}"
            )
        R = R_fix.copy()
    else:
        if verbose == 'on':
...
def CNMF_init(xdata,ydata,w,M,hyper,multi,delta,I_in,delta_h,delta_m,srf,init_mode=0,mask=0,verbose='off',MEMs=0):
    ...
    hx = int(np.floor(float(xdata) / float(w)))
    hy = int(np.floor(float(ydata) / float(w)))
    if verbose == 'on':
            ...
            for x in range(hx):
                for y in range(hy):
                    if x==0 or x==hx-1 or y==0 or y==hy-1:
                        ...
                        half = int(w // 2)
                        x0, x1 = int(x * w - half), int((x + 1) * w + half)
                        y0, y1 = int(y * w - half), int((y + 1) * w + half)
                        HSI[x,y,:] = (np.double(data[x0:x1, y0:y1, :]) * np.tile(H2, (1, 1, band))).sum(axis=0).sum(axis=0).reshape(1, 1, band)
        else:
            H1 = gaussian_filter2d((w,w),sig).reshape(w,w,1)
            ...
            for x in range(hx):
                for y in range(hy):
                    if x==0 or x==hx-1 or y==0 or y==hy-1:
                        ...
                        hlo = int((w - 1) // 2)
                        x0, x1 = int(x * w - hlo), int((x + 1) * w + hlo)
                        y0, y1 = int(y * w - hlo), int((y + 1) * w + hlo)
                        HSI[x,y,:] = (np.double(data[x0:x1, y0:y1, :]) * np.tile(H2, (1, 1, band))).sum(axis=0).sum(axis=0).reshape(1, 1, band)
    else: # with mask
        if np.mod(w,2)==0:
            H1 = gaussian_filter2d((w,w),sig).reshape(w,w,1)
...
def zoom_bi(data, w):
    ...
    rw = int(rows * w)
    cw = int(cols * w)
    r = np.tile(((2 * np.arange(0, rw, dtype=np.float64) + 1) / (2 * w) - 0.5).reshape(rw, 1), (1, cw))
    c = np.tile(((2 * np.arange(0, cw, dtype=np.float64) + 1) / (2 * w) - 0.5), (rw, 1))
    r[r<0] = 0
    r[r>rows-1] = rows-1
    c[c<0] = 0
    c[c>cols-1] = cols-1
...
def estR(HS,MS,mask=0):
    ...
    w = int(round(float(rows1) / float(rows2)))
    mask2 = zoom_nn(mask, w)
...
def vca(R,p):
    ...
    SNRth = 15 + 10 * np.log10(float(p)) + 8
...
def SAM(ref,tar,mask=0):
    ...
    USAGE
        sam_mean, angle_map = SAM(ref,tar)
    ...
    angle_map = np.real(np.arccos(prod_scal/prod_map))*180/np.pi
